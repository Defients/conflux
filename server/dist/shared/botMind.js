...
exports.simulateBotPerformance = simulateBotPerformance;
exports.decideBotPowerUp = decideBotPowerUp;
exports.decideBotOverdrive = decideBotOverdrive;
const types_1 = require("./types");
const seededRNG_1 = require("./seededRNG");
const constants_1 = require("./constants");
function applyRivalTraits(baseProfile, traits, dimension) {
    let profile = { ...baseProfile, reaction: { ...baseProfile.reaction }, typing: { ...baseProfile.typing }, precision: { ...baseProfile.precision } };
    for (const trait of traits) {
        switch (trait) {
            case types_1.RivalTraitId.PrecisionFocus:
                if (dimension === 'precision') {
                    profile.precision.star3Chance = Math.min(0.85, profile.precision.star3Chance + 0.15);
                }
                break;
            case types_1.RivalTraitId.ReactionPro:
                if (dimension === 'reaction' || dimension === 'rhythm') {
                    profile.reaction.mean = Math.max(100, profile.reaction.mean - 30);
                }
                break;
            case types_1.RivalTraitId.TypingAce:
                if (dimension === 'typing') {
                    profile.typing.wpm += 10;
                }
                break;
            case types_1.RivalTraitId.DebuffResistant:
                break;
            case types_1.RivalTraitId.AggressivePowerups:
                break;
        }
    }
    return profile;
}
function getBotResult(event, difficulty, botProfile, rng) {
    let primaryMetric = 0;
                ...
                while (pairsToFind > 0) {
                    ...
                    if (rng.nextFloat() < memoryChance && knownPairs > 0) {
                        ...
                        if (rng.nextFloat() < 0.2) {
                            ...
                            errors++;
                            if (rng.nextFloat() < memoryChance * 1.5)
                                knownPairs++;
                        }
                    }
                    if (clicks > 30)
    ...
    if (event.isStub) {
        return { stars: primaryMetric, primaryMetric: 0 };
    }
    return { stars, ...result };
}
function simulateBotPerformance(bot, event, difficulty, settings, rivalTraits) {
    if (!bot.personality) {
        return { stars: 1, primaryMetric: 0 };
    }
    let profile = constants_1.BOT_PROFILES[bot.personality];
    if (bot.isRival && rivalTraits && rivalTraits.length > 0) {
        profile = applyRivalTraits(profile, rivalTraits, event.performanceDimension);
    }
    const rng = new seededRNG_1.SeededRNG(`bot-${bot.id}-tile-${event.id}-${settings.seed}`);
    return getBotResult(event, difficulty, profile, rng);
}
function decideBotPowerUp(bot, gameState, currentTile) {
    if (bot.powerUps.length === 0 || currentTile.modifier === 'STATIC_FIELD')
        return null;
    const rng = new seededRNG_1.SeededRNG(`powerup-${bot.id}-tile-${gameState.currentTileIndex}-${gameState.settings.seed}`);
    // Easy bot misfire chance
    if (bot.personality === types_1.BotPersonality.Easy && rng.nextFloat() < 0.1) {
        return null;
    }
    ...
    if (offensivePowerUp) {
        ...
        let riskBias = bot.personality === types_1.BotPersonality.Intermediate ? 0.4 : 0.7;
        if (bot.isRival) {
...
function decideBotOverdrive(bot, gameState) {
    if (!bot.personality || bot.energy < 5 || bot.overdriveCooldown > 0)
        return false;
    const rng = new seededRNG_1.SeededRNG(`overdrive-${bot.id}-tile-${gameState.currentTileIndex}-${gameState.settings.seed}`);
    const playersSorted = [...gameState.players].sort((a, b) => b.position - a.position);
    const leader = playersSorted[0];
    ...
    if (bot.personality === types_1.BotPersonality.Intermediate) {
    ...
    if (bot.personality === types_1.BotPersonality.Easy) {
        const positionDifference = leader.position - bot.position;
...
//# sourceMappingURL=botMind.js.map