...
exports.computeMatchSummary = computeMatchSummary;
exports.applyMatchSummaryToProfile = applyMatchSummaryToProfile;
 ...
 */
function computeMatchSummary(input) {
    ...
    // ─── CP calculation ────────────────────────────────────────────────
    const cpPlacement = constants_1.CP_AWARD_RULES.placement[humanPlacement] ?? 0;
    const totalStars = humanPlayer.tileHistory.reduce((sum, tile) => sum + tile.stars, 0);
    let cpStars = totalStars * constants_1.CP_AWARD_RULES.perStar;
    ...
    const streakMultiplier = (0, constants_1.CP_STREAK_MULTIPLIER)(profile.winStreak);
    // ─── Rival W/L + streak ────────────────────────────────────────────
    ...
    const sponsorshipDeltas = [];
    const repRng = new seededRNG_1.SeededRNG(`rep-${settings.seed}`);
    ...
    // ─── Accolade checks ───────────────────────────────────────────────
    const accoladeUnlocks = [];
    const checkAccolade = (id) => {
        if (!profile.unlockedAccolades.includes(id)) {
            accoladeUnlocks.push(id);
        }
    };
    if (humanPlacement === 0)
        checkAccolade(types_1.AccoladeId.FirstVictory);
    if (rival && humanPlacement < rivalPlacement)
        checkAccolade(types_1.AccoladeId.RivalryBegins);
    const avgStars = humanPlayer.tileHistory.length > 0
        ...
        : 0;
    if (avgStars >= 3)
        checkAccolade(types_1.AccoladeId.Perfectionist);
    if (humanPlayer.tileHistory.some(h => h.stars === 4))
        checkAccolade(types_1.AccoladeId.Overdriver);
    const hazardTile = humanPlayer.tileHistory.find(h => {
        const tile = gameState.run[h.tileIndex];
        return tile && tile.isHazard;
    });
    if (hazardTile && hazardTile.stars >= 3)
        checkAccolade(types_1.AccoladeId.HazardousDuty);
    if (Object.values(types_1.ChassisId).every(id => profile.unlockedChassis.includes(id))) {
        checkAccolade(types_1.AccoladeId.Collector);
    }
    // ─── Rival delta ───────────────────────────────────────────────────
 ...
 */
function applyMatchSummaryToProfile(profile, summary) {
    // ─── Idempotency guard ──────────────────────────────────────────────
    ...
    if (!updated.appliedMatchIds)
        updated.appliedMatchIds = [];
...
// ─── Helpers ────────────────────────────────────────────────────────────────
function evaluateContractsForSummary(contracts, gameState, humanPlayer, sortedPlayers, eventDimensionMap) {
...
function createEmptySummary(matchId, mode, timestamp, seed, runLength, isGauntlet, isDaily) {
    return {
...
//# sourceMappingURL=matchSummary.js.map