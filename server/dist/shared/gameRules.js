...
const rivalBanter_1 = require("./rivalBanter");
// --- Helper Functions ---
const applyDebuff = (player, status, settings) => {
    // 1. Check Immunity
    ...
    if (player.statuses.some(s => s.type === 'SHIELDED')) {
        ...
        newStatuses.push({ type: 'IMMUNE', duration: constants_1.IMMUNITY_DURATION + 1 });
        const effect = (!player.isBot || player.isRival)
            ? { type: 'TOAST', message: `${player.name}'s Shield blocked the hit!`, variant: 'success' }
            : undefined;
        return { player: { ...player, statuses: newStatuses }, effect };
    }
    // 3. Priority Check
    const newPriority = constants_1.DEBUFF_PRIORITY[status.type] || 0;
    const currentPriority = Math.max(0, ...player.statuses.map(s => constants_1.DEBUFF_PRIORITY[s.type] || 0));
    ...
    if (chassis === types_1.ChassisId.GlassCannon) {
...
exports.GameRules = {
     ...
     */
    processRaceStep: (state, results, eventDescriptors) => {
        ...
        const powerUpRng = new seededRNG_1.SeededRNG(`powerup-award-${state.currentTileIndex}-${state.settings.seed}`);
        ...
        players = players.map(p => {
            ...
            // B. PowerUps
            const shouldAward = currentTile.modifier === 'POWER_SURGE' || (result.stars > 0 && constants_1.POWERUP_AWARD_RULES[result.stars]);
            if (shouldAward) {
                let award = constants_1.POWERUP_AWARD_RULES[result.stars] ?? 'Clarity';
                // Chassis Bonuses
                const chassis = p.chassisId ?? (p.isBot ? undefined : state.settings.selectedChassis);
                const shieldChance = chassis === types_1.ChassisId.Scavenger ? constants_1.SHIELD_UPGRADE_CHANCE + 0.2 : constants_1.SHIELD_UPGRADE_CHANCE;
                ...
                if (!p.isBot || p.isRival) {
                    ...
                    if (!p.isBot)
                        effects.push({ type: 'TOAST', message: `Acquired ${award}`, variant: 'info' });
                }
            }
            else {
                updatedPlayer.energy += result.stars;
            ...
            let moveMult = constants_1.STAR_MOVEMENT_MULTIPLIERS[result.stars] || 0;
            ...
            if (chassis === types_1.ChassisId.Momentum && updatedPlayer.statuses.some(s => s.type === 'BOOSTED'))
                moveMult *= 1.1;
            if (chassis === types_1.ChassisId.GlassCannon)
                moveMult *= 1.15;
            if (updatedPlayer.statuses.some(s => s.type === 'SLOWED'))
                moveMult *= 0.8;
            if (updatedPlayer.statuses.some(s => s.type === 'FROZEN' || s.type === 'STUNNED'))
                moveMult = 0;
            if (currentTile.modifier === 'BOOST_PAD' && result.stars >= 3) {
                moveMult *= 2;
                if (!p.isBot)
                    effects.push({ type: 'TOAST', message: 'Boost Pad Hit!', variant: 'success' });
            }
            ...
            // Momentum Activation
            if (chassis === types_1.ChassisId.Momentum && result.stars >= 3 && !updatedPlayer.statuses.some(s => s.type === 'BOOSTED')) {
                updatedPlayer.statuses = [...updatedPlayer.statuses, { type: 'BOOSTED', duration: 999 }];
                if (!p.isBot)
                    effects.push({ type: 'TOAST', message: 'Momentum Charged!', variant: 'success' });
            }
        ...
        players = players.map(p => {
            ...
            if (leaderTiles - playerTiles >= constants_1.RUBBER_BAND_THRESHOLD) {
                if (!p.isBot)
                    effects.push({ type: 'TOAST', message: 'Catch-up Boost Active', variant: 'info' });
                return { ...p, position: Math.min(100, p.position + baseStep * constants_1.RUBBER_BAND_BOOST) };
            }
            return p;
        });
        // 4. Status Management
        ...
        if (newLeader.id !== oldLeaderId) {
            if (newLeader.isRival) {
                const banter = (0, rivalBanter_1.getRivalBanter)('takeLead', `${state.settings.seed}-${state.currentTileIndex}`);
        ...
        if (rival) {
            ...
            if (humanRank <= 1)
                chance = 0.6;
            if (humanRank >= players.length - 2)
                chance = 0.2;
            const rng = new seededRNG_1.SeededRNG(`intervention-${state.currentTileIndex}-${state.settings.seed}`);
            ...
            if (nextIndex < state.run.length && canTrigger && rng.nextFloat() < chance) {
                ...
                activeIntervention = { type: 'HAZARD', hazardTile, cpBonus: constants_1.CP_AWARD_RULES.hazardBonus };
        ...
        if (state.activeAnomaly?.id === 'CHRONOS_SHIFT') {
            ...
            if (nextIndex < newRun.length - 1) {
                const rng = new seededRNG_1.SeededRNG(`chronos-${state.currentTileIndex}-${state.settings.seed}`);
     ...
     */
    applyPowerUp: (state, playerId, powerUp, targetId, eventDescriptors) => {
        const currentTile = state.run[state.currentTileIndex];
        ...
        if (!user || !user.powerUps.includes(powerUp))
        ...
        if (user.isRival) {
            const banter = (0, rivalBanter_1.getRivalBanter)('usePowerUp', `${state.settings.seed}-${state.currentTileIndex}`);
            effects.push({ type: 'RIVAL_TAUNT', message: `"${banter}"` });
        }
        else if (!user.isBot) {
        ...
        const rng = new seededRNG_1.SeededRNG(`quantum-${state.currentTileIndex}-${state.settings.seed}-${playerId}`);
        switch (powerUp) {
            case 'Shield':
            ...
            case 'Clarity':
                players = players.map(p => {
                    if (p.id === playerId) {
                        ...
                        if (hasBlur && !p.isBot)
                            effects.push({ type: 'TOAST', message: 'Vision Restored', variant: 'success' });
                    ...
                    players = players.map(p => {
                        if (p.id === target.id) {
                            ...
                            if (res.effect)
                                effects.push(res.effect);
                            if (!p.isBot)
                                effects.push({ type: 'SOUND', sound: 'debuff-hit' });
                            return res.player;
                        }
                        return p;
                    });
                    ...
                    players = players.map(p => {
                        if (p.id === spikeTargetId) {
                            ...
                            const scrambledRun = descriptors.length > 0
                                ? (0, pathGenerator_1.generateRun)(`scramble-${state.settings.seed}-${state.currentTileIndex}`, state.settings.runLength, descriptors)
                                : [];
                            if (res.effect)
                                effects.push(res.effect);
                            if (!p.isBot)
     ...
     */
    processPitStop: (state, playerId, action) => {
        let players = [...state.players];
        const player = players.find(p => p.id === playerId);
        if (!player)
            return { newState: state, effects: [] };
        const config = constants_1.PIT_STOP_CONFIG.actions[action];
        ...
        switch (action) {
            case 'scrub':
                ...
                break;
            case 'tuneUp':
                const tuneRng = new seededRNG_1.SeededRNG(`pitstop-${state.currentTileIndex}-${playerId}`);
                const keys = Object.keys(constants_1.POWERUP_AWARD_RULES);
                const award = constants_1.POWERUP_AWARD_RULES[keys[tuneRng.nextInt(0, keys.length)]];
     ...
     */
    activateOverdrive: (state, playerId, force = false) => {
        ...
        if (!force) {
            ...
            if (player.energy < constants_1.OVERDRIVE_ENERGY_COST) {
        ...
        const players = state.players.map(p => p.id === playerId
            ? { ...p, energy: force ? p.energy : p.energy - constants_1.OVERDRIVE_ENERGY_COST, overdriveCooldown: force ? p.overdriveCooldown : constants_1.OVERDRIVE_COOLDOWN + 1 }
            : p);
        return {
     ...
     */
    resolveIntervention: (state, accept) => {
